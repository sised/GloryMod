        ...
        public override void PreUpdate()
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if(npc.type == NPCID.WallofFlesh && Mworld.Menace && npc.active)
                {
                    ...
                    if (Main.npc[npc.whoAmI].life < Main.npc[npc.whoAmI].lifeMax / 2 && player.ZoneUnderworldHeight)
                    {
                        if (!HealingDisabled)
                        {
                            CombatText.NewText(new Rectangle((int)player.position.X, (int)player.position.Y, 40, 20), Color.Red, "Healing Disabled!", true);
                        }
                        HealingDisabled = true;
                        player.AddBuff(BuffID.PotionSickness, 5);
                    }
                    else HealingDisabled = false;
                    ...
                    if(distance > Main.npc[npc.whoAmI].GetGlobalNPC<GlobalNPCe>().BlazingRadious + Main.npc[npc.whoAmI].GetGlobalNPC<GlobalNPCe>().BlazingRadiusOffset && player.ZoneUnderworldHeight)
                    {
