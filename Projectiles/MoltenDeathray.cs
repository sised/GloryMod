...
namespace Glorymod.Projectiles
{
	...
	public class MoltenDeathray : ModProjectile
	{
		int iCarry;
		int warning;
		bool Exist = false;
		...
		private const float MOVE_DISTANCE = 120f;
		...
		public override void AI()
		{
			...
			if (projectile.active)
			{
				...
				for (int i = 0; i < Main.maxNPCs; i++)
				{
					...
					if (npce.type == NPCID.WallofFlesh && npce.active && !Main.npc[i].GetGlobalNPC<GlobalNPCe>().LaserAttached)
					{
						if (!Exist)
						{
							Exist = true;
				...
				float change = 0.3f;
				projectile.velocity = npce.velocity * change;
	...
	public class MoltenDeathray2 : ModProjectile
	{
		int iCarry;
		int warning;
		bool Exist = false;
		...
		public override void AI()
		{
			if (projectile.active)
			{
				...
				for (int i = 0; i < Main.maxNPCs; i++)
				{
					...
					if (npce.type == NPCID.WallofFlesh && npce.active)
					{
						if (!Exist)
						{
							Exist = true;
				...
				projectile.velocity = (npce.velocity * change).RotatedBy(MathHelper.ToRadians(projectile.ai[0]));
