            ...
            if (player.HasMinionAttackTargetNPC)
            {
                ...
                if (between < 2000f)
                {
                    ...
                    if (timer > 50)
                    {
                        Vector2 noscope = npc.Center - projectile.Center;
                        noscope.Normalize();
                        int B = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, noscope.X * 10f, noscope.Y * 10f, ModContent.ProjectileType<TerrorBeam>(), projectile.damage, 5, Main.myPlayer);
                        Main.projectile[B].ai[0] = 1;
                        timer = 0;
                    }
                }
            }
